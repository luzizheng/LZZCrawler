...
- (void)setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder
{
    ...
    [_imageview sd_setImageWithPreviousCachedImageWithURL:url placeholderImage:placeholder options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
        ...
        if (error) {
            ...
            CGFloat btn_w = 220;
            NSString * msg = [NSString stringWithFormat:@"原图加载失败，点击重新加载\nimage src:%@",url];
            CGRect msgRect = [msg boundingRectWithSize:CGSizeMake(btn_w, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:14]} context:nil];

            button.bounds = CGRectMake(0, 0, btn_w, msgRect.size.height+20);
